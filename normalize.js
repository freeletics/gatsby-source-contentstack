...
const buildBlockCustomSchema = (blocks, types, references, groups, fileFields, parent, prefix) => {
  const blockFields = {};
  ...
  blocks.forEach(block => {
    ...
    const {
      fields
    } = buildCustomSchema(block.schema, types, references, groups, fileFields, newparent, prefix);
...
const buildCustomSchema = exports.buildCustomSchema = (schema, types, references, groups, fileFields, parent, prefix) => {
  const fields = {};
  groups = groups || [];
  ...
  schema.forEach(field => {
    switch (field.data_type) {
      ...
      case 'global_field':
        ...
        const result = buildCustomSchema(field.schema, types, references, groups, fileFields, newparent, prefix);
      ...
      case 'blocks':
        ...
        const blockType = buildBlockCustomSchema(field.blocks, types, references, groups, fileFields, blockparent, prefix);
