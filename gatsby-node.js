...
exports.sourceNodes = async ({
  ...
  const {
    status
  } = store.getState(); // use a custom type prefix if specified
  ...
  configOptions.downloadImages && (await cache.set(SUPPORTED_FILES_COUNT, countOfSupportedFormatFiles)); // syncData.asset_published && syncData.asset_published.forEach((item) => {
  ...
  syncData.content_type_deleted && syncData.content_type_deleted.forEach(item => {
    item.content_type_uid = item.content_type_uid.replace(/-/g, '_');
    const sameContentTypeNodes = getNodes().filter(n => n.internal.type === `${typePrefix}_${item.content_type_uid}`);
    sameContentTypeNodes.forEach(node => deleteNode({
      node
    }));
  }); // Updating the syncToken

  const nextSyncToken = contentstackData.sync_token; // Storing the sync state for the next sync
...
exports.createResolvers = ({
  ...
  fileFields.forEach(fileField => {
    resolvers[fileField.parent] = { ...resolvers[fileField.parent],
      ...{
        [fileField.field.uid]: {
  ...
  references.forEach(reference => {
    resolvers[reference.parent] = { ...resolvers[reference.parent],
      [reference.uid]: {
        resolve(source, args, context, info) {
  ...
  groups.forEach(group => {
    resolvers[group.parent] = { ...resolvers[group.parent],
      ...{
        [group.field.uid]: {
          resolve: source => {
