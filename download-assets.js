...
module.exports = async ({
  ...
  try {
    ...
    const batches = getBatches(assets.length, configOptions.MAX_CONCURRENCY_LIMIT); // Get total count of files that will be downloaded, excluding unsupported formats

    totalJobs = await cache.get(SUPPORTED_FILES_COUNT); // Create progress bar
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      for (let j = skip; j < lastCount; j++) {
        ...
        let matches; // SVG is not supported by gatsby-source-filesystem. Reference: https://github.com/gatsbyjs/gatsby/issues/10297
        ...
        if (matches && !isUnsupportedExt) {
          batchPromises.push(await createRemoteFileNodePromise({
            cache,
            getCache,
            createNode,
            createNodeId
    ...
    reporter.info('Something went wrong while downloading assets. Details: ' + error); // throw error;
  }
};
...
const createRemoteFileNodePromise = async (params, node, typePrefix, reporter) => {
  try {
    ...
    let fileNode;
    const assetUid = makeAssetNodeUid(node, params.createNodeId, typePrefix); // Get asset from cache

    fileNode = await params.cache.get(assetUid); // Handles condition if the asset has been updated, then it will be downloaded again

    if (fileNode && fileNode.updated_at !== node.updated_at) fileNode = null;

    if (!fileNode) {
      fileNode = await createRemoteFileNode({ ...params,
    ...
    reporter.info('Something went wrong while creating file nodes, Details: ' + error); // throw error;
  }
};

const getBatches = (count, batchLimit) => {
  const partitions = Math.ceil(count / batchLimit); // Returns array filled with indexes
